...
Background:
  ...
  And   I select "LAPNET MC only" from 1st Course dropdown :pending
  And   I select "LAPNET MC only" from 2nd Course dropdown :pending

Scenario: Next
  When  I click Next :pending
  Then  I see "Re-confirm carefully..." :pending
  And   I see "Exam ID 1234" :pending
  And   I see "Course LAPNET MC only" :pending
