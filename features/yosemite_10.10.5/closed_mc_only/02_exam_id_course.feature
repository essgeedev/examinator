...
Scenario: Patty's path to success
  ...
  And   I select CLOSED MC only from 1st Course dropdown :pending
  And   I select CLOSED MC only from 2nd Course dropdown :pending
  When  I click Next :pending
  Then  I see Re-confirm carefully... :pending
  And   I see Exam ID 1234 :pending
  And   I see CLOSED MC only :pending
